...
- **Aksesibilitas**: kontras teks memenuhi WCAG AA di tema terang dan gelap (dijaga oleh tes token warna); navigasi penuh dengan keyboard (skip link, fokus terlihat, editor SQL tidak menjebak tombol Tab, dialog dengan focus trap dan Escape); landmark, `aria-current`, peran tab, dan pengumuman hasil untuk pembaca layar; status benar/salah tidak hanya lewat warna; animasi confetti mengikuti `prefers-reduced-motion`; halaman bisa di-zoom.
- **Cheat Sheet SQL 📖 & tur onboarding 🧭**: referensi sintaks per topik (dengan pencarian, dan catatan beda SQLite/MySQL) bisa dibuka dari header quest dan Playground. Tur 6 langkah muncul otomatis satu kali saat pertama masuk Level 1 (menjelaskan tab Quest, Editor, Schema, Result, hint, dan cheat sheet) dan bisa diulang lewat tombol 🧭. Contoh di cheat sheet memakai nama generik, bukan jawaban soal.
- **Diagram ERD 🔗**: panel Schema punya tombol **Daftar | Diagram**. Diagram menggambar tabel, kolom PK/FK, dan relasi berkardinalitas (1 / N) dari FOREIGN KEY sungguhan — atau, bila belum dideklarasikan, diperkirakan dari nama kolom (garis putus-putus). Klik/Enter pada tabel menyorot relasinya, **Perbesar** membukanya layar penuh, dan diagram ikut berubah saat mahasiswa membuat tabel baru (mis. hasil normalisasi di Level 10). Pembaca layar mendapat deskripsi teks lengkap.
- **Verifikasi progress tanpa backend 🛡️**: file Export kini menyimpan SQL yang meloloskan tiap tahap dan bertanda tangan HMAC. Di Rekap Kelas dosen, tombol *Verifikasi Ulang Semua* menjalankan ulang SQL itu dengan penilai yang sama di sandbox baru: nilai yang hanya diketik di JSON (tanpa SQL yang benar-benar bekerja) ditandai, begitu juga file yang diubah setelah diekspor, XP yang tidak cocok dengan log, tahap yang lulus terlalu cepat, dan jawaban panjang yang identik antar mahasiswa. **Batasan yang jujur:** ini situs statis — kunci tanda tangan dan kunci jawaban ikut terkirim ke browser, jadi orang yang membaca kode sumber tetap bisa memalsukan. Ini penangkal perubahan iseng dan alat bantu tinjau sampel, bukan bukti; untuk nilai resmi tetap gunakan ujian terawasi. Kunci tanda tangan ada di [js/integrity.js](js/integrity.js) (ganti bila perlu).
- **Soal Prediksi Output 🔮** (Level 5–8): mahasiswa membaca sebuah query lalu memilih hasilnya *tanpa menjalankannya* — jebakan seperti urutan `ORDER BY`, `WHERE` sebelum `GROUP BY`, `AVG` yang mengabaikan `NULL`, syarat di `ON` pada `LEFT JOIN`, dan `NOT IN` dengan `NULL`. Hasil sebenarnya baru ditampilkan setelah menjawab benar. Kunci jawaban diverifikasi otomatis terhadap sandbox oleh tes, dan soal ikut masuk antrean Review Konsep bila salah.
- **Susun Potongan SQL 🧩** (Level 5–8): mahasiswa menyusun potongan query yang diacak — sebagian pengecoh — dengan tombol ketuk / ↑ ↓ (nyaman di HP dan keyboard, tanpa drag & drop). Penilaian berdasarkan *hasil* query yang tersusun, bukan urutan kanonik tunggal, dan tes memastikan setiap pengecoh membuat query gagal di posisi mana pun ia disisipkan.
- **Efficiency dinilai dari rencana eksekusi ⚡**: tantangan Optimasi (Level 11 dan 14) tidak menilai teks query, melainkan `EXPLAIN QUERY PLAN` yang benar-benar dipilih database setelah perintah mahasiswa berjalan — mis. `SCAN` (seluruh tabel dibaca) harus berubah menjadi `SEARCH … USING INDEX`, atau `USE TEMP B-TREE FOR ORDER BY` harus hilang. Rencana ditampilkan di panel Result beserta arti tiap istilah; hasil query tetap harus benar. Tahap bernilai Level 11 (buat index) kini juga dicek lewat rencana, jadi index di kolom yang salah tidak lagi lolos. Komponen *Interpretation* belum dinilai (tetap disembunyikan).
- **Latihan Tambahan yang dilipat ➕**: soal opsional (prediksi output, susun potongan SQL, debug, optimasi) dikelompokkan dalam satu bagian *Latihan Tambahan* di daftar tahap dan tidak lagi menghalangi jalur utama — tombol "Lanjut" melompati mereka, masuk level langsung ke tahap utama berikutnya, dan penomoran hanya untuk tahap utama. Tahap opsional tetap memberi XP dan bisa dibuka kapan saja.
